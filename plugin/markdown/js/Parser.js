...
(function () {
  ...
  Parser = (function () {
    ...
    htmlspecialchars = function (str, needParse) {
      // 转义html
      return needParse ? str.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;') : str
    }
    ...
    trim = function (str, ch) {
      ...
      if (ch != null) {
        search = ''
        for (i = j = 0, ref = ch.length - 1; ref >= 0 ? j <= ref : j >= ref; i = ref >= 0 ? ++j : --j) {
          c = ch[i]
          c = preg_quote(c)
          search += c
        }
    ...
    function Parser (config) {
      ...
      this.parsers = {}
      this._config = config || {
        parseHTML: true, // false
        newLine: true, // false
        yourTags: '' // code-wrap|template
      }
    }
    ...
    Parser.prototype.call = function () {
      ...
      type = arguments[0], args = arguments.length >= 2 ? slice.call(arguments, 1) : []
      value = args[0]
      if (this.hooks[type] == null) {
        return value
      }
    ...
    Parser.prototype.optimizeLines = function (html) {
      ...
      if (this.line) {
        return html.replace(regex, function () {
          var matches, replace
          matches = arguments.length >= 1 ? slice.call(arguments, 0) : []
    ...
    Parser.prototype.parseInline = function (text, whiteList, clearHolders, enableAutoLink) {
      ...
      text = text.replace(/(^|[^\\])(`+)(.+?)\2/mg, (function (_this) {
        return function () {
          var matches
          matches = arguments.length >= 1 ? slice.call(arguments, 0) : []
          return matches[1] + _this.makeHolder('<code>' + (htmlspecialchars(matches[3])) + '</code>', _this._config.parseHTML)
        }
      })(this))
      text = text.replace(/(^|[^\\])(\$+)(.+?)\2/mg, (function (_this) {
        return function () {
          var matches
          matches = arguments.length >= 1 ? slice.call(arguments, 0) : []
          return matches[1] + _this.makeHolder(matches[2] + (htmlspecialchars(matches[3])) + matches[2], _this._config.parseHTML)
        }
      })(this))
      text = text.replace(/\\(.)/g, (function (_this) {
        return function () {
          ...
          escaped = htmlspecialchars(matches[1], _this._config.parseHTML)
      ...
      text = text.replace(/<(\/?)([a-z0-9-]+)(\s+[^>]*)?>/ig, (function (_this) {
        return function () {
          ...
          if (_this.html || (('|' + _this.commonWhiteList + '|' + whiteList + '|').indexOf('|' + matches[2].toLowerCase() + '|')) >= 0) {
            ...
            return _this.makeHolder(htmlspecialchars(matches[0]), _this._config.parseHTML)
          }
        }
      })(this))
      if (this.html) {
      ...
      text = text.replace(/!\[((?:[^\]]|\\\]|\\\[)*?)\]\(((?:[^\)]|\\\)|\\\()+?)\)/g, (function (_this) {
        return function () {
          ...
          matches = arguments.length >= 1 ? slice.call(arguments, 0) : []
          escaped = htmlspecialchars(_this.escapeBracket(matches[1]), _this._config.parseHTML)
    ...
    Parser.prototype.parseBlockList = function (block, key, line, state) {
      ...
      if (matches = line.match(/^(\s*)((?:[0-9]+\.)|\-|\+|\*)\s+/)) {
        space = matches[1].length
        tab = matches[0].length - space
        state.empty = 0
        type = '+-*'.indexOf(matches[2]) >= 0 ? 'ul' : 'ol'
    ...
    Parser.prototype.parseBlockAhtml = function (block, key, line, state) {
      ...
      if (this.html) {
        htmlTagRegExp = new RegExp('^\\s*<(' + this.blockHtmlTags + ')(\\s+[^>]*)?>')
        if (matches = line.match(htmlTagRegExp)) {
          if (this.isBlock('ahtml')) {
            ...
            htmlTagAllRegExp = new RegExp('\\s*<(' + this.blockHtmlTags + ')(\\s+[^>]*)?>', 'g')
            while (true) {
        ...
        } else if (!!state.html && line.indexOf('</' + state.html + '>') >= 0) {
          this.setBlock(key).endBlock()
          state.html = false
          return false
    ...
    Parser.prototype.parseBlockHtml = function (block, key, line, state) {
      var matches, tag
      if (matches = line.match(new RegExp('^\\s*<(' + state.special + ')(\\s+[^>]*)?>'))) {
        tag = matches[1].toLowerCase()
        if (!(this.isBlock('html', tag)) && !(this.isBlock('pre'))) {
          this.startBlock('html', key, tag)
        }
        return false
      } else if (matches = line.match(new RegExp('</(' + state.special + ')>\\s*$'))) {
        tag = matches[1].toLowerCase()
    ...
    Parser.prototype.parseCode = function (lines, parts, start) {
      ...
      lines = lines.slice(1, -1).map(function (line) {
        ...
        return htmlspecialchars(line, _this._config.parseHTML)
      })
    ...
    Parser.prototype.parsePre = function (lines, value, start) {
      ...
      lines = lines.map(function (line) {
        return htmlspecialchars(line.substring(4), _this._config.parseHTML)
      })
    ...
    Parser.prototype.parseNormal = function (lines, inline, start) {
      ...
      str = trim(lines.join('\n'))
      // /^\\s*<(slot|template|Button|code-wrap)(\\s+[^>]*)?>/

      addWrap = new RegExp('^\\s*<(|/)(' + this._config.yourTags + ')(\\s+[^>]*)?>').test(str)
      ...
      // 添加换行
      if (this._config.newLine) str = str.replace(/\n/g, '<br>')
      if (str.match(/^\s*$/)) {
        return ''
      } else {
        if (inline) {
    ...
    Parser.prototype.cleanUrl = function (url, parseTitle) {
      ...
      if (parseTitle) {
        pos = url.indexOf(' ')
        if (pos >= 0) {
          title = htmlspecialchars(trim(url.substring(pos + 1), ' "\''), this._config.parseHTML)
    ...
    Parser.prototype.isBlock = function (type, value) {
      ...
      return this.current === type && (value === null ? true : this.blocks[this.pos][3] === value)
    }
